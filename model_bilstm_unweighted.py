...
glove_words = pickle.load(open(f'embeddings/840B.300_words.pkl', 'rb'))
glove_word2idx = pickle.load(open(f'embeddings/840B.300_idx.pkl', 'rb'))
glove_embs = pickle.load(open(f'embeddings/840B.300_embs.pkl', 'rb'))
...
class BiRNN(nn.Module):
    ...
    def __init__(self, vocab_size, target_vocab, output_size, embedding_dim, hidden_dim, n_layers, drop_prob=0.5):
        ...
        #     try:
        #         if v in ['_PAD','_UNK']:
        #             weights_matrix[word2idx[v]] = torch.from_numpy(glove_embs[0])
        #         else:
        #             weights_matrix[word2idx[v]] = torch.from_numpy(glove_embs[glove_word2idx[v]])
        #     except:
        #         pass
    ...
    for inputs, labels in train_loader:
        ...
        if counter%print_every == 0:
            ...
            if np.mean(val_losses) < valid_loss_min:
                torch.save(model.state_dict(), 'models/' + domain + '/un' + weighted + '_' + dataamount + '_' + str(seed) + '.pt')
...
# f = open('logs/' + domain + '/un' + str(seed) + '_' + weighted + '_' + dataamount + '.csv', 'w+')
...
model.load_state_dict(torch.load('models/' + domain + '/un' + weighted + '_' + dataamount + '_' + str(seed) + '.pt'))
